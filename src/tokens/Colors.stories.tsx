...
const PRIMITIVE: { label: string; tokens: string[] }[] = [
  { label: 'Brand', tokens: ['--color-brand-shinhan-blue'] },
  {
    ...
    tokens: [
      ...
      '--color-primary-700', '--color-primary-800', '--color-primary-900',
    ],
  },
  {
    label: 'Greyscale',
    tokens: [
      '--color-greyscale-200', '--color-greyscale-300', '--color-greyscale-400',
      '--color-greyscale-500', '--color-greyscale-550', '--color-greyscale-600',
      '--color-greyscale-700', '--color-greyscale-800', '--color-greyscale-900',
    ],
  },
  {
    label: 'Neutral',
  ...
  { label: 'State', tokens: ['--color-state-red', '--color-state-green'] },
  {
    label: 'Label',
    tokens: [
      '--color-label-purple-deep', '--color-label-green-deep',
      '--color-label-orange-deep', '--color-label-neutral-light-gray',
    ],
  },
  {
    label: 'Gauge',
    tokens: ['--color-gauge-blue', '--color-gauge-green', '--color-gauge-purple'],
  },
...
const SEMANTIC: string[] = [
  ...
  '--color-fg', '--color-fg-muted', '--color-on-primary',
  '--color-danger', '--color-success', '--color-overlay',
]
...
function Swatch({ name }: { name: string }) {
  ...
  return (
    <div className="flex flex-col gap-1">
      ...
      <code className="text-caption2-regular text-fg-muted uppercase">{value}</code>
    </div>
  )
}
...
export const Semantic: Story = {
  render: () => (
    <div className="flex flex-col gap-3 bg-bg p-8">
      <Section label="Semantic (역할 기반 · Tailwind 유틸 생성)" tokens={SEMANTIC} />
    </div>
  ),
}
