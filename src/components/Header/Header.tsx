...
function LogoArrow() {
  return (
    <div
      ...
      style={{ width: 'var(--layout-lnb-w)', borderColor: 'var(--color-greyscale-200)' }}
    >
...
function Breadcrumb({ items }: { items: string[] }) {
  return (
    <div className="flex flex-1 flex-col justify-center px-12" style={{ height: 'var(--layout-header-h)' }}>
      <div className="flex items-center gap-2" style={{ color: 'var(--color-greyscale-500)' }}>
        {items.map((item, idx) => (
          <span key={idx} className="flex items-center gap-2">
            {idx > 0 && (
              <Icon name="small-right" size={24} color="current" />
            )}
...
function ProjectBox({
  ...
  const showTag = type === 'pcMainOn' || type === 'pcMainOff' || type === 'devmode'
  ...
  return (
    <div className="flex items-start">
      <div
        ...
        style={{ backgroundColor: background, borderColor: 'var(--color-greyscale-300)' }}
      >
...
function RightActions({
  ...
  return (
    <div className="flex items-center justify-center gap-4 px-8 py-4">
      ...
      <span className="relative flex" style={{ color: 'var(--color-greyscale-900)' }}>
      ...
      <span
        ...
        style={{ backgroundColor: 'var(--color-brand-shinhan-blue)', color: 'var(--color-text-white)' }}
      >
        {userInitial}
      </span>
    </div>
