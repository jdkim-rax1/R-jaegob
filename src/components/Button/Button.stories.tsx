...
const meta: Meta<typeof Button> = {
  ...
  args: { variant: 'primary', size: 'md', state: 'enabled', children: 'Button' },
}
export default meta
...
export const Sizes: Story = {
  render: (args) => (
    <div className="flex items-center gap-4">
      <Button {...args} size="lg">Button</Button>
      <Button {...args} size="md">Button</Button>
      <Button {...args} size="sm">Button</Button>
    </div>
  ),
}
    ...
    <div className="flex flex-col gap-4">
      {(['enabled', 'pressed', 'disabled'] as const).map((state) => (
        <div key={state} className="flex items-center gap-4">
          ...
          <Button variant="secondary" state={state}>Button</Button>
          <Button variant="primary" state={state}>Button</Button>
        </div>
      ))}
    </div>
  ),
